...
def run_script(cmd, cwd='.'):
    if len(cmd) > 0 and cmd[0].startswith("#"):
        print(f"[ComfyUI-Manager-DZ] Unexpected behavior: `{cmd}`")
        return 0
...
try:
    ...
    print(f"## ComfyUI-Manager-DZ: installing dependencies")
    ...
    try:
        ...
        print(f"## [ERROR] ComfyUI-Manager-DZ: Attempting to reinstall dependencies using an alternative method.")
        run_script([sys.executable, '-s', '-m', 'pip', 'install', '--user', '-r', requirements_path])

        try:
            import git
        except:
            print(f"## [ERROR] ComfyUI-Manager-DZ: Failed to install the GitPython package in the correct Python environment. Please install it manually in the appropriate environment. (You can seek help at https://app.element.io/#/room/%23comfyui_space%3Amatrix.org)")

    print(f"## ComfyUI-Manager-DZ: installing dependencies done.")
...
default_channels = 'default::https://raw.gitmirror.com/ltdrdata/ComfyUI-Manager/main,recent::https://raw.gitmirror.com/ltdrdata/ComfyUI-Manager/main/node_db/new,'
...
def read_config():
    try:
        ...
        if channel_url_list_is_valid:
            ...
            print(f"[WARN] ComfyUI-Manager-DZ: channel_url_list is invalid format")
            ch_url_list = ''
...
def try_install_script(url, repo_path, install_cmd):
    ...
    if platform.system() == "Windows" and int_comfyui_revision >= comfy_ui_required_revision:
        ...
        print(f"\n## ComfyUI-Manager-DZ: EXECUTE => {install_cmd}")
        ...
        if platform.system() == "Windows":
            try:
                if int(comfy_ui_revision) < comfy_ui_required_revision:
                    print("\n\n###################################################################")
                    print(f"[WARN] ComfyUI-Manager-DZ: Your ComfyUI version ({comfy_ui_revision}) is too old. Please update to the latest version.")
...
def print_comfyui_version():
    ...
    try:
        ...
        try:
            if int(comfy_ui_revision) < comfy_ui_required_revision:
                print(f"\n\n## [WARN] ComfyUI-Manager-DZ: Your ComfyUI version ({comfy_ui_revision}) is too old. Please update to the latest version. ##\n\n")
        except:
            pass
...
def setup_js():
    import nodes
    js_dest_path = os.path.join(js_path, "comfyui-manager-dz")

    if hasattr(nodes, "EXTENSION_WEB_DIRS"):
        if os.path.exists(js_dest_path):
            shutil.rmtree(js_dest_path)
    else:
        print(f"[WARN] ComfyUI-Manager-DZ: Your ComfyUI version is outdated. Please update to the latest version.")
        # setup js
        if not os.path.exists(js_dest_path):
            os.makedirs(js_dest_path)
        js_src_path = os.path.join(comfyui_manager_path, "js", "comfyui-manager.js")

        print(f"### ComfyUI-Manager-DZ: Copy .js from '{js_src_path}' to '{js_dest_path}'")
...
def download_chunk(url, start_byte, end_byte, file_name, num):
    ...
    with open(file_name, 'r+b') as file:
        ...
        #file.write(data)
        pbar = tqdm(total=(end_byte-start_byte), unit='B', unit_scale=True, desc=file_name.split("\\")[-1]+' Part '+str(num))
        while True:
...
def merge_file(file_name, num_threads):
    with open(file_name, 'wb') as output_file:
        for i in range(num_threads):
            with open(f'{file_name}_{i}', 'rb') as input_file:
                output_file.write(input_file.read())

def download_chunk2(url, start, end):
    ...
    chunk = response.read()
    return chunk
def download_file_thread(url, local_filename):
    if githubFlag:
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response :
            ...
            if total_size < 409600000:
                num_threads = 4
            if total_size < 102400000:
                num_threads = 2
            with open(local_filename, 'wb'):
                pass
            ...
            threads = []
            for i in range(num_threads):
                thread = threading.Thread(target=download_chunk, args=(url, ranges[i][0], ranges[i][1], local_filename,i))
        ...
        print(f"Download error: {url} / {e}", file=sys.stderr)
        return False
    print("Mission completed !!!     " , local_filename.split("\\")[-1])
    return True

def download_file(url, local_filename):
    #opener = urllib.request.build_opener()
    #proxy_handler = urllib.request.ProxyHandler({'http': 'http://172.16.96.4:8123', 'https': 'http://172.16.96.4:8123'})
    ...
    try:
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3'}
        req = urllib.request.Request(url, headers=headers)
        with urllib.request.urlopen(req) as response, open(local_filename, 'wb') as f:
            total_size = int(response.headers['Content-Length'])
            pbar = tqdm(total=total_size, unit='B', unit_scale=True, desc=local_filename)
            while True:
                chunk = response.read(4096)
                if not chunk:
                    break
...
def gitclone_install(files, mirror=False):
...
@server.PromptServer.instance.routes.post("/model/install_dz")
async def install_model(request):
    json_data = await request.json()
    model_path = get_model_path(json_data)
    res = False
    try:
        #json_data['desc'] = json_data['description']
        #json_data['description'] = Translator(from_lang="english",to_lang="chinese").translate(json_data['description'])
        if model_path is not None:
            print(f"Install model '{json_data['mirror']}' '{json_data['url']}' into '{model_path}'")
            if json_data['url'].startswith('https://github.com') or json_data['url'].startswith('https://huggingface.co'):
                ...
                if json_data['mirror'] == "mirror":
                    download_file_thread(json_data['url'], model_path)
                else:
...
WEB_DIRECTORY = "js"
NODE_CLASS_MAPPINGS = {}
__all__ = ['NODE_CLASS_MAPPINGS_DL']
